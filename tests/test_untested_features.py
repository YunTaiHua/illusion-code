...
async def test_plugins_load():
    ...
    from illusion.plugins.loader import load_plugin
...
async def test_memory_lifecycle():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        mem_dir = Path(tmpdir) / ".illusion" / "memory"
...
async def test_config_paths():
    ...
    # All should be under ~/.illusion
    all_under_home = all(".illusion" in str(p) for p in paths.values())
