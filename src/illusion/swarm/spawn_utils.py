...
def get_teammate_command() -> str:
    ...
    Resolution order:
    1. ``ILLUSION_TEAMMATE_COMMAND`` environment variable — allows the
       operator to point at a specific binary or wrapper script.
    2. The ``illusion`` entry-point on PATH (installed package mode).
    3. The current Python interpreter running the ``illusion`` module
...
def build_inherited_env_vars() -> dict[str, str]:
    ...
    Always includes ``ILLUSION_AGENT_TEAMS=1`` plus any provider/proxy
